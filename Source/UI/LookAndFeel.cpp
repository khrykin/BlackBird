...
void KnobLookAndFeel::drawRotarySlider(Graphics &g, int x, int y, int width,
  ...
  auto dotsAngleIncrement =
      (rotaryEndAngle - rotaryStartAngle) / (numberOfDots - 1.0f);
  auto dotSize = 2.0f;
  auto dotsRadius = jmin(width / 2, height / 2) - dotSize;
  auto knobRadius = dotsRadius * 0.6f;

  if (slider.getName() ==
      OscillatorKnob::sliderName) { // Draw oscillator type knob
    auto numberOfLines = (int)icons.size();
    auto lineLength = 10.0f;
    auto lineGap = 5.0f;

    auto linesAngleIncrement =
        (rotaryEndAngle - rotaryStartAngle) / (numberOfLines - 1.0f);
    ...
    for (auto i = 0; i < numberOfLines; i++) {
      ...
      line.applyTransform(
          AffineTransform::translation(centerX, centerY - knobRadius - lineGap -
                                                    lineLength)
      ...
      imageOrigin.addXY((float)-1.0f * icon->getWidth() +
                            1.5f * lineGap * std::sin(lineAngle),
                        (float)-1.5f * icon->getHeight() -
                            lineGap * std::cos(lineAngle));

      auto effect = DropShadowEffect();
      effect.setShadowProperties(oscShadow);

      if (!isHighlighted) {
        effect.setShadowProperties(
            DropShadow(Colours::transparentWhite, 1, juce::Point(0, 0)));
      }

      icon->setComponentEffect(&effect);
      icon->draw(g, 1.0f,
                 AffineTransform::scale(1.0f).translated(imageOrigin.getX(),
                                                         imageOrigin.getY()));
    }
  } else { // Draw dots
    ...
    for (auto i = 0; i < numberOfDots; i++) {
      ...
      Path p;
      p.addEllipse(-dotSize, -dotSize, 2 * dotSize, 2 * dotSize);
      p.applyTransform(
          AffineTransform::translation(centerX, centerY - dotsRadius)
  ...
  { // Draw a shadow
    ...
    Path shadow;
    shadow.addEllipse(centerX - knobRadius, centerY - knobRadius,
                      2 * knobRadius, 2 * knobRadius);

    shadow.addEllipse(centerX - knobRadius - shadowShift, centerY - knobRadius,
                      2 * knobRadius, 2 * knobRadius);
    shadow.addRectangle(centerX - shadowShift, centerY - knobRadius,
                        shadowShift, 2 * knobRadius);

    shadow.applyTransform(
        AffineTransform::rotation(shadowAngle, centerX, centerY));
  ...
  { // Draw a knob
    ...
    g.fillEllipse(centerX - knobRadius, centerY - knobRadius, 2 * knobRadius,
                  2 * knobRadius);
  }
  ...
  { // Draw a knob's pointer
    ...
    Path pointer;
    pointer.addRoundedRectangle(-lineThickness * 0.5f, -knobRadius,
                                lineThickness, pointerLength,
                                0.5f * lineThickness);
    ...
    pointer.applyTransform(
        AffineTransform::rotation(pointerAngle).translated(centerX, centerY));
...
Label *KnobLookAndFeel::createSliderTextBox(Slider &slider) {
  ...
  l->setColour(Label::outlineColourId,
               slider.findColour(Slider::textBoxOutlineColourId));
  l->setColour(TextEditor::textColourId,
               slider.findColour(Slider::textBoxTextColourId));
  l->setColour(
  ...
  l->setColour(TextEditor::outlineColourId,
               slider.findColour(Slider::textBoxOutlineColourId));
...
void KnobLookAndFeel::fillTextEditorBackground(Graphics &g, int width,
                                               int height, TextEditor &editor) {
...
void KnobLookAndFeel::drawTextEditorOutline(Graphics &g, int width, int height,
                                            TextEditor &editor) {
  //    g.drawRoundedRectangle(1, 1, width - 2, height - 2, 0.5f * height, 2.0);
}

void HeaderLookAndFeel::drawComboBox(Graphics &g, int width, int height,
  ...
  g.setColour(Colour(21, 22, 22)
                  .withMultipliedSaturation(isButtonDown ? 1.3f : 0.9f)
...
void HeaderLookAndFeel::drawPopupMenuBackground(Graphics &g, int width,
                                                int height) {
  g.setColour(Colour(21, 22, 22));
  g.fillAll();
}
...
void HeaderLookAndFeel::drawButtonBackground(Graphics &g, Button &button,
...
void HeaderLookAndFeel::drawButtonText(Graphics &g, TextButton &button,
                                       bool shouldDrawButtonAsHighlighted,
                                       bool shouldDrawButtonAsDown) {
  ...
  Rectangle<int> arrowZone(0.5 * (bounds.getWidth() - arrowWidth), 0,
  ...
  g.setColour(Colour(220, 220, 220)
                  .withAlpha((shouldDrawButtonAsHighlighted ? 0.9f : 0.2f)));
...
Font getDefaultFont() {
  static auto defaultFont = Typeface::createSystemTypefaceFor(
      BinaryData::Jost600Semi_ttf, BinaryData::Jost600Semi_ttfSize);
  return defaultFont;
}

Font getDefaultBoldFont() {
  static auto defaultBoldFont = Typeface::createSystemTypefaceFor(
      BinaryData::Jost700Bold_ttf, BinaryData::Jost700Bold_ttfSize);
