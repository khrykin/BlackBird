...
BlackBirdAudioProcessorEditor::BlackBirdAudioProcessorEditor(
    BlackBirdAudioProcessor &p, AudioProcessorValueTreeState &valueTreeState)
    : AudioProcessorEditor(&p), processor(p), valueTreeState(valueTreeState) {
  setupControls();

  addAndMakeVisible(header);

  setSize(8.0f * masterSection.recommendedWidth() + 1.0f * padding +
              3.0f * 0.5f * padding,
...
void BlackBirdAudioProcessorEditor::setupControls() {
  ...
  addKnobToSection(oscillatorSection, oscillatorKnob,
                   oscillatorWaveformParameterID);
  addParameterAsKnobToSection(oscillatorSection, characterParameterID,
                              "Character");

  addAndMakeVisible(filterSection);
  addParameterAsKnobToSection(filterSection, filterCutoffParameterID, "Cutoff");
  addParameterAsKnobToSection(filterSection, filterResonanceParameterID,
                              "Resonance");
  ...
  addParameterAsKnobToSection(envelopeSection, cutoffEnvelopeAmountParameterID,
                              "C-off Amount");
  addParameterAsKnobToSection(
      envelopeSection, resonanceEnvelopeAmountParameterID, "Reso. Amount");
  addParameterAsKnobToSection(envelopeSection,
                              velocityEnvelopeAmountParameterID, "Vel. Amount");
...
Knob *BlackBirdAudioProcessorEditor::addParameterAsKnobToSection(
    Section &section, const String &parameterID, const String &title) {
...
void BlackBirdAudioProcessorEditor::addKnobToSection(
    Section &section, Knob *knob, const String &parameterID) {
  sliderAttachments.emplace_back(std::make_unique<SliderAttachment>(
      valueTreeState, parameterID, knob->slider()));
...
void BlackBirdAudioProcessorEditor::resized() {
  ...
  auto items = {&oscillatorSection, &filterSection, &envelopeSection,
                &masterSection};

  std::for_each(std::begin(items), std::end(items), [&](Section *section) {
