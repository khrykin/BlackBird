...
Knob::~Knob() { setLookAndFeel(nullptr); }

#pragma mark - Acessing Private Instances

Slider &Knob::slider() { return _slider; }

#pragma mark - Geometry

float Knob::minWidth() { return _widthFactor * defaultSize; }

float Knob::minHeight() { return defaultSize; }

int Knob::widthFactor() const { return _widthFactor; }

void Knob::setWidthFactor(int width) { _widthFactor = width; }
...
void Knob::paint(Graphics &g) {
  ...
  g.drawFittedText(getName().toUpperCase(), 0, rect.getHeight() - labelHeight,
                   rect.getWidth(), labelHeight, Justification::centred, 2);
}
...
OscillatorKnob::OscillatorKnob(const String &title, unsigned int width)
    : Knob(title, width) {
  _slider.setTextBoxStyle(Slider::NoTextBox, false, 50, 16);
