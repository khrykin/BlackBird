...
#pragma mark - Static Properties
  ...
  static constexpr auto minEnvelopeDurationSeconds =
      Voice::minEnvelopeDurationSeconds;
  static constexpr auto maxEnvelopeDurationSeconds =
      Voice::maxEnvelopeDurationSeconds;
...
#pragma mark - Default Properties Values
  ...
  static constexpr auto defaultCutoffEnvelopeAmount =
      Voice::defaultCutoffEnvelopeAmount;
  static constexpr auto defaultResonanceEnvelopeAmount =
      Voice::defaultResonanceEnvelopeAmount;
  static constexpr auto defaultVelocityEnvelopeAmount =
      Voice::defaultVelocityEnvelopeAmount;
...
#pragma mark - Reverb
  ...
  inline bool reverbIsOn() const { return *parameters.reverb != 0.0f; }
...
#pragma mark - Rendering Audio Output
  ...
  void renderVoices(AudioBuffer<float> &outputBuffer, int startSampleIndex,
    ...
    outputBuffer.applyGainRamp(startSampleIndex, numSamples, lastMasterGain,
                               *parameters.masterGain);

    lastMasterGain = *parameters.masterGain;
  }

  void applyMasterFxChain(AudioBuffer<float> &outputBuffer,
                          int startSampleIndex, int numSamples) {
    ...
    outputBuffer.applyGainRamp(startSampleIndex, numSamples,
                               1.0 - lastReverbGain, 1.0 - *parameters.reverb);
